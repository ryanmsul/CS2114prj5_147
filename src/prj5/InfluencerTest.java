...
// ----------------------------------------------------------
    ...
    public void testGetTraditionalEngagementRateParameter()
    {
        assertEquals(
            ...
            0.1);
        assertEquals(-1, iAZero.getTraditionalEngagementRate
            (monthZero.getMonth()), 0.1);
    }
    ...
    public void testGetTraditionalEngagementRate()
    {
        ...
        assertEquals(
            ((3.0 + 3.0) / 1.0) * 100.0,
            iA.getTraditionalEngagementRate(),
            0.1);
        assertEquals(-1, iAZero.getTraditionalEngagementRate(), 0.1);
        assertEquals(
            ((24 + 6) / 6.0) * 100.0,
            iB.getTraditionalEngagementRate(),
            0.1);
    ...
    {
        Influencer iAZero2 =
            new Influencer("A name", "A channel", "France", "Frogs");

        iAZero2.addMonthActivity(monthZero);

        assertEquals(
            ((3.0 + 3.0) / 3.0) * 100.0,
            iA.getReachEngagementRate(),
            0.1);
        assertEquals(-1, iAZero2.getReachEngagementRate(), 0.1);
        assertEquals(
            ((24 + 6) / 30.0) * 100.0,
            iB.getReachEngagementRate(),
            0.1);
    }
    ...
    public void testGetReachEngagementRateParameter()
    {
        assertEquals(
            ...
            0.1);
        assertEquals(-1, iAZero.getReachEngagementRate
            (monthZero.getMonth()), 0.1);
    }
        ...
        throws MonthException
    {
        ...
        Influencer iADiffChannelName =
            new Influencer("A name", "A diffChannel", "France", "Frogs");

        assertFalse(iA.equals(iADiffChannelName));

        Influencer iADiffPageTopic =
            new Influencer("A name", "A channel", "France", "Bears");
